...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!result.success) {
    return res.status(400).json({ error: result.error.issues[0]?.message || "Invalid booking payload" });
  }
  ...
  try {
    ...
    return res.status(200).json({ ok: true, message: "Booking received. We will contact you shortly." });
  } catch (error) {
    console.error("Booking API failed", error);
    return res.status(500).json({ error: "Unable to process booking request right now." });
  }
}
